...
\begin{itemize}
	\item FDMA (Freq Division Multiple Access): rozdělení kmitočtového pásma na jendotlivé kanály $\rightarrow$ každý uživatel dostane přidělenou svou vlastní frekvenci
	...
	\item ALOHA: data se odešlou kdykoliv; pokud se 2 signály "srazí", vysílači přijde potvrzení o nepřijetí a data se odešlou znovu s náhodným časovým zpožděním znovu
\end{itemize}

\section{Metody multiplexování}
Princip: z jednoho bodu se odesílá více různých signálů do jednoho radiového kanálu (např.: rádio s mnoha stanicemi)
\begin{itemize}
	...
	\begin{description}
		...
		\item[$\circleddash$] přesná sycnhronizace (může vznikat zpoždění)
	\end{description} 
...
\begin{figure}[H]
	...
	\caption{Multiplexování v rekvenční, časové a kódové doméně}
	\label{fig:multiplexovani}
\end{figure}

\section{Proces rozprostírání signálu a výhody/nevýhody}
Jev typický pro CDMA. Užitečný signál se násobí rozprostíracím šumovým kódem $\rightarrow$ umělé zvětšení šířky pásma a výkon se rozloží do širšího spektra. Na přijímači se stejný kód použije pro zpětné zúžení.\\
Důvody použití:
\begin{itemize}
...
\begin{itemize}
	...
	\item mnohacestným šířením mezi vysílačem a přijímačem, komplikovaným navíc pohybem TX a RX (vektorové sčítání signálů)
\end{itemize}
\begin{figure}[H]
	\centering
...
\begin{itemize}
	...
	\end{figure}
	\newpage
	\item \textbf{Buňková struktura} - území rozděleno na velké množství menších oblastí (buněk). Uvnitř buňky je umístěna základová radiová stanice. Buňky mají různou velikost i tvar:
	\begin{itemize}	
...
\end{itemize}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.6\linewidth]{"Obrazky/MKO/5. otazka/bunkovaStruktura2"}
...
\section{Handover}
Proces, při kterém se mobilní spojení přepojí na jinou mobilní stanici, když se uživatel pohybuje mezi buňkami bez přerušení komunikace. Kdyby nedošlo k předání spojení, mobilní stanice by při opuštění buňky ztratila spojení.\\
\textbf{Průběh:}
\begin{enumerate}
...
\begin{itemize}
	...
	\end{figure}
	\item \textbf{Měkký handover} paralelní spojení s více kanály
	\begin{figure}[H]
		\centering
...
\subsection{GSM 2G}
GSM navržen tak, aby nebyl uzavřený, ale aby umožňoval přístup i do jiných sítí (pevná telefonní linka, mobilní sítě, např. LTE).\\
...
\subsection{UMTS 3G}
Oproti GSM/GPRS/EDGE přenáší výrazně vyšší kapacitu, lepší datové služby a širší možnost mobiilního přístupu k síti\\
Architektura:
\begin{itemize}
...
Cílem LZE: zvýšení přenosové rychlosti, zkrácení odezvy, snížení nákladů\\
...
\subsection{Modulace:}
\begin{itemize}
	\item \textbf{Vnější (rozděluje kanál)} 
	\begin{itemize}	
		\item OFDM (rozdělení širokého 20 MHz spektra na několik subnosných)
	\end{itemize}
	\item \textbf{Vnitřní (zapisuje data na subnosné)}
	\begin{itemize}	
...
Jednotlivé prvky architektury:
\begin{itemize}
	\item \textbf{eNodeB} - základnová stanice (anténa), řídící jendotka rádiové sítě
	\begin{itemize}	
		\item modulace
...
\begin{itemize}
	...
	\item Zdrojové prostředky (RB) jsou přidělovány uživatelům na základě jejich požadavků, aktuálním stavu sítě a časové kmitočtové mpy úniků
	\item Nejmenší kmitočtovou časovou jendotkou je zdrojový blok RB (resource block)
	\item Nejmenší přenosovou jendotkou je SB (scheduling block), tvořený dvěma PRB, které na sebe bezprostředně časově navazují (12 subnosných $\times$ 0,5 ms)
	...
	\begin{itemize}	
		...
		\item \textbf{Synchronizační signál} - identifikační buňky (synchronizace UE se sítí, aby mohl azačít komunikace)
		\begin{figure}[H]
			\centering
...
Varianty se liší: rychlostí přneosu, efektivitou, podporou víceuživatelského provozu
\begin{figure}[H]
	\centering
...
\begin{itemize}	
	...
	\begin{itemize}	
		\item rozprostření spektra pseudonáhodnou sekvencí za použití logické funkce XOR na šířku pásma 20 MHz
...
Přístup pomocí paketů \textbf{RTS} (request to send) a CTS (clear to send).\\
...
\begin{itemize}
	...
	\begin{itemize}	
		...
		\item Komunikace probíhá přes \textit{Mastera} k zařízení \textit{Slave} (\textit{Slave} zařízení nemohou spolu komunikovat napřímo)
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item IoT zařízení má vlastní software, senzory a možnost síťové konektivity.
...
\begin{itemize}
	\item Využití rozprostření spektra
