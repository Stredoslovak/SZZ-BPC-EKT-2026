...
\chapter{BPC-TVT - Televizní technika a multimedia}
\begin{huge}
	Otázky:
	\newline
\end{huge}
...
\end{enumerate}
\newpage
\section{První otázka}
\subsection{Základní poznatky fotometrie}
Fotometrie je věda, která se zabývá měřením vlastností světla
\subsubsection{veličiny a jednotky pro popis světla}
...
\subsection{Televizní kolorimetrie}
...
\end{itemize}
\subsubsection{Kolorimetrická soustava RGB}
...
\subsubsection{Kolorimetrická soustava XYZ}
...
\subsubsection{Diagram barev MKO}
...
\begin{figure}[H]
	...
	\label{fig:mko}
\end{figure}
\subsubsection{normalizované barevné pruhy}
...
\section{Druhá otázka}
\subsection{Základy televizního přenosu}
...
\subsubsection{Rozklad a diskretizace obrazového toku}
...
\begin{itemize}
	...
	\end{itemize}
\end{itemize}
\subsubsection{barevný obrazový signál}
...
\]
Barevný televizní přenos musí být slučitelný s černobílým. Proto se přenáší:
\begin{itemize}
	\item \textbf{jasový signál} \(U_Y\),
	\item \textbf{chrominanční signály} \(U_R-U_Y\) a \(U_B-U_Y\).
\end{itemize}
Plný barevný signál BS obsahuje jasový signál, barevné informace a synchronizační složky. Barevné informace se přenášejí pomocí dvou chrominančních signálů, protože barevná rozlišovací schopnost oka je menší než rozlišovací schopnost pro jas. Proto mohou mít barevné signály menší šířku pásma než signál jasový.\\
Pro obraz barevných pruhů se ve skriptech ukazuje, že bílá barva vznikne při \(U_R = U_G = U_B = 1\), černá při nulových hodnotách a ostatní barvy odpovídají kombinacím základních složek
\begin{figure}[H]
	\centering
...
\end{figure}
Synchronizační impulz barvy SIB (burst) se v TV přijímači používá k synchronizaci oscilátoru, který generuje barvonosnou složku, která se používá pro demodulaci barvonosného kanálu. Impulz se vkládá do zadní části zatemňovacího impulzu (ten největší peak ohraničující barevné pásmo; řídící signál, který určuje začátek obrazového intervalu; vysoký, aby jej přijímač spolehlivě poznal).

\subsection{Televizní přenosová soustava}
\subsubsection{DVB}
...
\subsubsection{TV pásma a šířky jejich kanálů}
Způsoby přenášení televizního signálu:
\begin{itemize}
	\item \textbf{Pozemní vysílání} - radiovými vlnami
	\item \textbf{Kabelovým přenosem}
	\item \textbf{Družicovým vysíláním}
\end{itemize}
...
\section{Třetí otázka}
\subsection{Snímání obrazu a televizní obrazovky}
...
\subsubsection{princip snímače CCD}
\begin{itemize}
...
\subsubsection{provedení snímačů a jejich vlastnosti}
Struktura snímače:
\begin{itemize}
...
\end{itemize}
\begin{itemize}
	\item \textbf{FT (Frame Transfer)} -- obsahuje snímací část a paměťovou část; obrazový náboj se po skončení expozice přenese do paměťové části. Nevýhodou může být mazaní obrazu ve svislém směru při silném osvětlení.
	\item \textbf{LT (Line Transfer)} -- mezi sloupci světlocitlivých buněk jsou vloženy vertikální posuvné registry, čímž se zmenšuje mazaní obrazu, ale snižuje se využití plochy snímače.
...
\begin{figure}[H]
	...
	\caption{Princip snímačů: FT, LT, FIT}
	\label{fig:ftalt}
\end{figure}
...
\subsubsection{Obrazovky s tekutými krystaly LCD}
...
Zěna napětí způsobí změnu natočení molekul a tím i změnu propustnosti polarizovaného světla.
\begin{figure}[H]
	\centering
...
\subsubsection{Plazmové obrazovky}
...
\subsubsection{OLED zobrazovače}
...
\subsubsection{Srovnání zobrazovacích technologií (principy a vlastnosti)}
...
\section{Čtvrtá otázka}
\subsection{Televizní informační systémy}

\subsubsection{úrovně teletextu}
...
\subsubsection{multimediální platforma MHP }
...
\subsubsection{multimediální platforma HbbTV}
...
\subsection{Kódování multimediálních signálů}
...
\subsubsection{Kanálové kódování}
...
\subsubsection{Digitalizace obrazového toku}
...
\section{Pátá otázka}
\subsection{Zdrojové kódování obrazových signálů}
Výsledkem zdrojového kódování je zmenšení datového toku při co nejmenším zhoršení subjektivní kvality obrazu.
Provádíme za účelem odstranění (u řeči, zvuku, obrazu, videu):
\begin{itemize}
	\item Redundance (nadbytečná data) - data, co se opakují/lze je odvodit od ostatních; vratný proces, \underline{bezztrátová komprese}
	\item Irelevance (zbytečná data) - data, která lidské smysly nedokáží vnímat, nebo nejsou důležité; nevratný proces \underline{ztrátová komprese}
\end{itemize}
\subsubsection{redukce redundance a irelevance}
Redukce redundance: 
\begin{itemize}
...
\subsubsection{vzorkovací formáty}
...
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{BMA} (block amtching algorythm) - použití pohybových vektorů určujících jak
...
\subsubsection{principy JPEG a MPEG-1/2/AVC/HEVC}
\begin{itemize}
	...
	\begin{itemize}
		...
		\item oproti MPEG-1 zpracovává signál i ve formátu 4:2:2 (makroblok složen ze 4 bloků signálu (16x16) a dvou dovjic bloků chrominančního signálu)
...
\subsubsection{predikce a vektory pohybu}
více viz výše: Komprimace videa\\
...
\section{Šestá otázka}
\subsection{Zdrojové kódování zvukových signálů}
...
\subsubsection{Frekvenční a časové maskování}
...
\begin{itemize}
    \item \textbf{Frekvenční maskování} -- při současném vnímání dvou zvukových signálů déle než 200\,ms může silnější signál potlačit slyšitelnost slabšího signálu na jiném kmitočtu. Závislost prahu slyšitelnosti na kmitočtu a úrovni maskujícího signálu popisují \textbf{křivky současné slyšitelnosti}.
    \item \textbf{Časové maskování} -- maskování nastává i tehdy, když maskovaný signál přichází až do 10\,ms po ukončení maskujícího signálu (postmaskování), nebo do 5\,ms před jeho nástupem (premaskování). Po 200\,ms maskování zcela zaniká.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{"Obrazky/TVT/6. otazka/prahy"}
...
\subsection{Komprimace audia -- principy MPEG}
...
\subsubsection{Zvukový standard MPEG-1}
\begin{itemize}
...
\subsubsection{Zvukový standard MPEG-2 a prostorový zvuk}
...
\section{Sedmá otázka}
\subsection{Multiplexování}
...
\subsubsection{Elementární toky (Elementary Streams -- ES)}
...
\subsubsection{Paketizovaný elementární tok -- PES}
...
\subsubsection{Programový tok (Program Stream -- PS)}
...
\subsubsection{Transportní tok (Transport Stream -- TS)}
...
\subsubsection{Vnější kódování FEC1 -- Reed-Solomonův kód}
\begin{itemize}
...
\subsubsection{Vnitřní kódování FEC2 -- konvoluční kód}
\begin{itemize}
...
\subsubsection{Prokládání (Interleaving)}
...
\subsubsection{M-PSK (Phase Shift Keying)}
...
\subsubsection{M-QAM (Quadrature Amplitude Modulation)}
...
\subsubsection{OFDM (Orthogonal Frequency Division Multiplex)}
...
\subsubsection{Volba ochranného intervalu}
...
\subsubsection{Volba parametrů kódování}
...
\subsubsection{Standard DVB-T2 a rotovaná konstelace}
...
\subsubsection{Přijímací antény pro DVB-T}
\begin{itemize}
...
\section{Devátá otázka}
\subsection{Družicové televizní vysílání a standardy DVB-S/S2}

\subsubsection{Geostacionární dráha a výhody družicového vysílání}
...
\subsubsection{Frekvenční pásma uplinku a downlinku}
...
\subsubsection{Modulace a standardy DVB-S/S2}
\begin{itemize}
...
\subsubsection{Družicový transpondér}
...
\subsubsection{Přijímací anténa}
...
\subsubsection{Vnější jednotka -- LNB}
...
\subsubsection{Vnitřní jednotka a protokol DiSEqC}
...
\section{Desátá otázka}
\subsection{Televizní přijímače DVB}

\subsubsection{Bloková koncepce přijímače DVB}
...
\subsubsection{IDTV a set-top-box}
\begin{itemize}
...
\subsubsection{Vstupní díl a tuner}
...
\subsubsection{Dálkové ovládání}
\begin{itemize}
...
\subsection{Standardy pro přenos s vysokým rozlišením}

\subsubsection{Formáty HDTV}
...
\subsubsection{Perspektiva přenosu UHDTV}
